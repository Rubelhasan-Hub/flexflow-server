...
// API For trainer----------------------
app.get('/api/forum-posts/by-author/:email', async (req, res) => {
    try {
...
app.get('/api/all-classes', async (req, res) => {
    try {
        const { status, search, category, page = 1, limit = 6 } = req.query;
        const query = {};
...
app.get('/api/forum-posts', async (req, res) => {
    try {
        ...
        const [posts, total] = await Promise.all([
            forumPostsCollection.find().sort({ createdAt: -1 }).skip(skip).limit(limitNum).toArray(),
            forumPostsCollection.countDocuments()
        ]);
